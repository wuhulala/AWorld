...
class MCPServerExecutor:
    ...
    async def call_tool(
        ...
        logger.info(
            f"Calling tool: server={self._name}, tool_name={tool_name}, raw_tool_name={name}, args={arguments}"
        )
        result = await self._session.call_tool(
            tool_name, arguments, progress_callback=progress_callback_proxy
        )
        ...
        try:
            ...
            vnc_tool_card = {
                ...
                "card_data": {
                    ...
                    "url": f"/novnc/{get_vnc_session_key()}/vnc_lite.html?scale=true&show_top_bar=false",
                },
            }
            message = f"""\
\n\n
...
"""
    ...
    async def _start_tool_server(
        ...
        async with self._lock:
            ...
            try:
                ...
                max_retry = int(config.get("max_retry", "30"))
                for i in range(max_retry):
                    exit_stack = AsyncExitStack()
                    try:
                        await exit_stack.__aenter__()
                        session = await self._init_client_session(
                            name, config, exit_stack
                        )
                        tools = await session.list_tools()
                        logger.info(
                            f"Starting tool server success! {name}: {map(lambda x: x.name, tools.tools)}"
                        )
                        ...
                        logger.error(
                            f"Error starting tool server {name} (retry {i+1}/{max_retry}): {traceback.format_exc()}"
                        )
                        try:
                    ...
                    msg = f"Error starting tool server {name} after {max_retry} retries"
                    logger.error(msg)
                    raise Exception(msg)
    ...
    async def _init_client_session(
        self, name: str, config: dict, exit_stack: AsyncExitStack
    ):
        ...
        if config.get("type") == "sse":
            ...
            env = config.get("env", {})
            env.update({k: v for k, v in os.environ.items() if k in ["DISPLAY"]})
        ...
        async def log_callback(params: LoggingMessageNotificationParams):
            logger.info(f"MCP Server [{name}] Log >>> {params}")

...
class ContainerServerManager:
    """
    Container server manager is responsible for managing container server instances and env pod instances.
    """
    ...
    async def create_container(
        ...
        async with httpx.AsyncClient() as client:
            try:
                vpc_session_id = (
                    f"sid-{session_id.session_id}"
                    if session_id.session_id
                    else f"msid-{session_id.mcp_session_id}"
                )
                ...
                if ret.pop("status") == "success":
                    return (
                        ...
                        ret.get("data", {}).get("channel_port"),
                        ret.get("data", {}).get("http_port"),
    ...
    async def shutdown_container(
        self, container_server_id: str, container_id: str, trace_id: str = "NA"
    ) -> bool:
        async with httpx.AsyncClient() as client:
            try:
    ...
    async def list_containers(self, container_server_id: str) -> List[dict]:
        """List all containers of a container server"""
        container_server = await self._container_server_repo.get_server(
            container_server_id
        )
        if not container_server:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"http://{container_server.ip_addr}:{container_server.port}/api/container/list",
                json={"token": container_server.token},
                timeout=httpx.Timeout(60.0),
            )
            response.raise_for_status()
            body = response.json()
            ...
            ), f"Failed to list containers of container server {container_server_id}: {body.get('message', 'Unknown error')}"
...
async def container_server_manager_builder():
    ...
    if redis_url:
        container_server_repo = ContainerServerRedisRepo(redis_url)
        await container_server_repo.initialize()
    else:
        container_server_repo = ContainerServerRepo()

    container_server_manager = ContainerServerManager(container_server_repo)

...
@mcp.tool(description="查询与指定节点相关的知识，使用图数据库进行关联查询")
async def get_related_entities(node_id: str, max_depth: int = 2, limit: int = 10, session_id: str = None) -> Union[str, TextContent]:
    """
    查询与指定节点相关的知识，使用图数据库进行关联查询。
    
    此工具通过图数据库查询与指定节点相关的知识，支持多层级关联查询。
    
    Args:
        node_id (str): 要查询关联知识的节点ID
        session_id (str, optional): 会话ID，用于标识工作空间
        max_depth (int, optional): 最大查询深度，默认为2
        limit (int, optional): 返回结果数量限制，默认为10
        
    Returns:
        Union[str, TextContent]: 关联知识内容，或错误信息
        
    Raises:
        None: 返回错误信息字符串而不是抛出异常
    """
    logger.info(f"🔍 查询关联知识: node_id={node_id}, session_id={session_id}, max_depth={max_depth}, limit={limit}")
    
    try:
        # 获取图数据库实例
        graph_db = graph_db_factory.get_graph_db()
        if not graph_db:
            logger.warning("⚠️ 图数据库未配置或不可用")
            return "图数据库未配置或不可用，无法进行关联查询"
        
        # 初始化图数据库
        await graph_db.initialize()
        
        # 检查节点是否存在
        if not await graph_db.has_node(namespace=session_id, node_id=node_id):
            logger.warning(f"⚠️ 节点不存在: node_id={node_id}")
            return f"节点 {node_id} 不存在"
        
        # 批量获取关联节点的详细信息
        related_nodes = await graph_db.get_related_nodes(namespace=session_id, node_id=node_id, max_depth=max_depth, limit=limit)
        
        # 构建返回结果
        result_content = f"与节点 {node_id} 相关的知识 (共找到 {len(related_nodes)} 个关联节点):\n\n"
        
        for node_id_found, node_data in related_nodes.items():
            result_content += f"<related_node>\n"
            result_content += f"<node_id>{node_id_found}</node_id>\n"
            
            # 添加节点属性信息
            if node_data:
                for key, value in node_data.items():
                    if key != 'id':  # 避免重复显示ID
                        result_content += f"<{key}>{value}</{key}>\n"
            
            result_content += f"</related_node>\n\n"
        
        search_output_dict = {
            "related_nodes": result_content,
            "related_nodes_count": len(related_nodes),
            "query_node": node_id
        }
        
        logger.info(f"✅ 成功查询到 {len(related_nodes)} 个关联节点")
        ...
        logger.error(f"❌ 查询关联知识时出错: node_id={node_id}, session_id={session_id}, error={str(e)}, trace={traceback.format_exc()}")
        return f"查询关联知识时出错: {str(e)}"


@mcp.tool(description="获取所有实体节点的ID列表")
async def get_entity_list(session_id: str = None, limit: int = 100, offset: int = 0) -> Union[str, TextContent]:
    """
    获取知识图谱中所有实体节点的ID列表。
    
    此工具返回指定会话工作空间中所有实体节点的ID列表，支持分页查询。
    
    Args:
        session_id (str, optional): 会话ID，用于标识工作空间
        limit (int, optional): 返回结果数量限制，默认为100
        offset (int, optional): 偏移量，用于分页查询，默认为0
        
    Returns:
        Union[str, TextContent]: 实体ID列表，或错误信息
        
    Raises:
        None: 返回错误信息字符串而不是抛出异常
    """
    logger.info(f"🔍 获取实体列表: session_id={session_id}, limit={limit}, offset={offset}")
    ...
    try:
        ...
        if not graph_db:
            logger.warning("⚠️ 图数据库未配置或不可用")
            return "图数据库未配置或不可用，无法获取实体列表"
        
        # 初始化图数据库
        await graph_db.initialize()
        
        # 获取所有节点
        all_nodes = await graph_db.get_all_nodes(namespace=session_id)
        ...
        # 构建返回结果
        result_content = f"实体节点列表 (共 {total_count} 个，显示第 {start_idx + 1}-{end_idx} 个):\n\n"
        
        entity_list = []
        for i, node in enumerate(paginated_nodes, start=start_idx + 1):
            node_id = node.get('id', f'unknown_{i}')
            node_type = node.get('type', 'unknown')
            node_name = node.get('name', node_id)
            
            entity_info = f"{i}. ID: {node_id}"
            if node_name != node_id:
                entity_info += f", 名称: {node_name}"
            if node_type != 'unknown':
                entity_info += f", 类型: {node_type}"
            
            entity_list.append(entity_info)
            result_content += f"<entity>\n{entity_info}\n</entity>\n"
        
        # 添加分页信息
        if total_count > limit:
            result_content += f"\n分页信息: 共 {total_count} 个实体，当前显示第 {start_idx + 1}-{end_idx} 个"
            if offset + limit < total_count:
                result_content += f"，还有 {total_count - end_idx} 个实体未显示"
        
        search_output_dict = {
            "entity_list": entity_list,
            "total_count": total_count,
            "returned_count": len(paginated_nodes),
            "limit": limit,
            "offset": offset,
            "has_more": end_idx < total_count
        }
        
        logger.info(f"✅ 成功获取 {len(paginated_nodes)} 个实体节点 (共 {total_count} 个)")
        ...
        logger.error(f"❌ 获取实体列表时出错: session_id={session_id}, error={str(e)}, trace={traceback.format_exc()}")
        return f"获取实体列表时出错: {str(e)}"

"""
ChromaDB vector database implementation for amnicontext.
...
class ChromaVectorDB(VectorDB):
    ...
    def search(
        ...
        try:
            ...
            if collection:
                ...
                where_filter = self.build_where_conditions(filter)
    ...
    def build_where_conditions(self, filter):
        where_conditions = []
        if filter:
    ...
    def _convert2_embedding_result_with_score(self, result, distances=None, threshold=None) -> Optional[list[SearchResult]]:
        ...
        for document, metadata, id, score in zip(
            ...
            logging.debug(f"search embedding_result_with_score {id}:{score}")
    ...
    def upsert(self, collection_name: str, items: list[EmbeddingsResult]):
        ...
        Args:
            collection_name (str): Name of the collection
            items (list[Amni]): List of embedding results to upsert
        """
        collection = self.client.get_or_create_collection(
            name=collection_name, metadata={"hnsw:space": "cosine"}
        )
